...
export function renderDigestText(samples: MetricSample[], alerts: Alert[]): string {
  ...
  const parts: string[] = [];
  parts.push(`📊 Datum 12-hour digest — ${new Date().toISOString().slice(0, 16).replace('T', ' ')} UTC`);
  parts.push('');
  parts.push(`Summary: ${samples.length} metrics tracked · ${dataAlerts.length} data alert(s) · ${techAlerts.length} technical issue(s)`);
  parts.push('');
  ...
  if (dataAlerts.length > 0) {
    parts.push(`📊 Data alerts (${dataAlerts.length}):`);
    for (const a of dataAlerts) {
      if (a.kind === 'metric') {
        parts.push(`   • ${a.dashboardName} — ${a.label}: ${formatValue(a.current, a.unit)} (${a.rule})`);
      }
    }
    parts.push('');
  }
  if (techAlerts.length > 0) {
    parts.push(`🔧 Technical issues (${techAlerts.length}):`);
    for (const a of techAlerts) {
      if (a.kind === 'technical') {
        const where = a.label ? `${a.dashboardName} → ${a.label}` : a.dashboardName;
        parts.push(`   • ${where}: ${a.message}`);
      }
    }
    parts.push('');
  }
  if (dataAlerts.length === 0 && techAlerts.length === 0) {
    parts.push('🟢 No alerts in the last 12 hours.');
  }
  return parts.join('\n');
}
...
export function renderDigestHtml(samples: MetricSample[], alerts: Alert[]): string {
  ...
  const dataBlock = dataAlerts.length > 0
    ? `<h3 style="margin-top:24px;">📊 Data Insights — fired in the last 12h</h3>
       <p style="color:#666;font-size:13px;margin:0 0 8px 0;">These are also pushed live to the Telegram channel.</p>
       ${dataAlerts.map(renderAlertHtml).join('')}`
    : '<h3 style="margin-top:24px;color:#2e7d32;">📊 Data Insights — none in the last 12h 🟢</h3>';

  const techBlock = techAlerts.length > 0
    ? `<h3 style="margin-top:24px;">🔧 Technical Issues — fired in the last 12h</h3>
       <p style="color:#666;font-size:13px;margin:0 0 8px 0;">These are batched here only — the Telegram channel does not receive them.</p>
       ${techAlerts.map(renderAlertHtml).join('')}`
    : '<h3 style="margin-top:24px;color:#2e7d32;">🔧 Technical Issues — none in the last 12h 🟢</h3>';

  return `
    <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:720px;margin:0 auto;padding:16px;">
      <h2 style="margin:0 0 8px 0;">Datum Labs — 12-hour digest</h2>
      <p style="color:#666;margin:0 0 4px 0;">${new Date().toUTCString()}</p>
      <p style="color:#666;margin:0 0 16px 0;font-size:13px;">
        ${samples.length} metrics tracked across ${byDashboard.size} dashboards ·
        ${dataAlerts.length} data alert(s) · ${techAlerts.length} technical issue(s)
      </p>
      ${rows.join('\n')}
      ${dataBlock}
      ${techBlock}
      <p style="color:#888;font-size:12px;margin-top:24px;">
        ...
        Routing: Data alerts → Telegram (live) + this email (every 12h). Technical issues → this email only.
      </p>
    </div>`;
}
